...
// Game action types for maze navigation
pub enum Action {
    Up,
    Down,
    Left,
...
// Action selector that chooses actions based on policy
...
impl ActionSelector {
    ...
    pub fn select_action(&self, q_values: &Array1<f64>) -> Action {
        match &self.strategy {
            ...
            ActionStrategy::Greedy => {
                // Deterministically choose the best action
                let mut best_action = 0;
                let mut best_value = q_values[0];
    ...
    pub fn update_exploration_rate(&mut self, new_rate: f64) {
        if let ActionStrategy::EpsilonGreedy { ref mut exploration_rate } = self.strategy {
