...
impl Brain {
    pub fn new(
        ...
        let weights1 =
            Array::from_shape_fn((hidden_size1, input_size), |_| normal.sample(&mut rng));
        let biases1 = Array::from_shape_fn(hidden_size1, |_| normal.sample(&mut rng));
        let weights2 =
            Array::from_shape_fn((hidden_size2, hidden_size1), |_| normal.sample(&mut rng));
        let biases2 = Array::from_shape_fn(hidden_size2, |_| normal.sample(&mut rng));
        let weights3 =
            Array::from_shape_fn((output_size, hidden_size2), |_| normal.sample(&mut rng));
    ...
    pub fn train(&mut self, input: &Array1<f64>, target: &Array1<f64>) {
        ...
        let hidden2_delta =
            &hidden2_error * &hidden2_activated.mapv(|h| if h > 0.0 { 1.0 } else { 0.01 });

        let hidden1_error = self.weights2.t().dot(&hidden2_delta);
        let hidden1_delta =
            &hidden1_error * &hidden1_activated.mapv(|h| if h > 0.0 { 1.0 } else { 0.01 });

        let output_delta_reshaped = output_delta
            .clone()
            .into_shape((output_delta.len(), 1))
            .unwrap();
        let hidden2_activated_reshaped = hidden2_activated
            .clone()
            .into_shape((1, hidden2_activated.len()))
            .unwrap();
        self.weights3 = &self.weights3
            + &(&output_delta_reshaped.dot(&hidden2_activated_reshaped) * self.learning_rate);
        self.biases3 = &self.biases3 + &(&output_delta * self.learning_rate);

        let hidden2_delta_reshaped = hidden2_delta
            .clone()
            .into_shape((hidden2_delta.len(), 1))
            .unwrap();
        let hidden1_activated_reshaped = hidden1_activated
            .clone()
            .into_shape((1, hidden1_activated.len()))
            .unwrap();
        self.weights2 = &self.weights2
            + &(&hidden2_delta_reshaped.dot(&hidden1_activated_reshaped) * self.learning_rate);
        self.biases2 = &self.biases2 + &(&hidden2_delta * self.learning_rate);

        let hidden1_delta_reshaped = hidden1_delta
            .clone()
            .into_shape((hidden1_delta.len(), 1))
            .unwrap();
        let input_reshaped = input.clone().into_shape((1, input.len())).unwrap();
        self.weights1 =
            &self.weights1 + &(&hidden1_delta_reshaped.dot(&input_reshaped) * self.learning_rate);
        self.biases1 = &self.biases1 + &(&hidden1_delta * self.learning_rate);
    }

    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<(), String> {
        let serialized =
            serde_json::to_string(self).map_err(|e| format!("Failed to serialize brain: {}", e))?;
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self, String> {
        ...
        serde_json::from_str(&contents).map_err(|e| format!("Failed to deserialize brain: {}", e))
    }
